...
function GenerateAI() {
  ...
  const reCraftRecipe        = useAppStore(selectReCraftRecipe);
  ...
  const addedLower = useMemo(
    () => new Set(aiIngredients.map((i) => i.toLowerCase())),
    [aiIngredients],
  );
      ...
      inputValue.trim().length === 0
        ...
        : SUGGESTIONS.filter(
            (s) =>
              ...
              !addedLower.has(s.toLowerCase()),
          ).slice(0, 8),
    [inputValue, addedLower],
  );
  ...
  }, [isGenerating, isRecrafting, reCraftRecipe]);
